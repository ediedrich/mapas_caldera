# -*- coding: utf-8 -*-
"""La toponimia del departamento en doce nóminas oficiales, 1909-1940,
...
print("en once o doce listas:", sum(1 for n,*r in T if sum(r[:NC])>=11))

# ------------------------------------------------------------------ FIGURA
import textwrap
nr = len(T)
fig = plt.figure(figsize=(8.27, 11.2))
ax  = fig.add_axes([0.180, 0.048, 0.478, 0.830])
ax2 = fig.add_axes([0.672, 0.048, 0.030, 0.830])
...
for lbl, (n, *r) in zip(ax.get_yticklabels(), T):
    if sum(r[:NC]) >= 11: lbl.set_weight('bold')
...
for i, (name, *r) in enumerate(T):
    ...
    n = sum(r[:NC])
    for j in range(NC):
        if r[j]: ax.plot(j, i, 's', ms=4.0, color='#1f4e5f' if n >= 11 else '#5a8ca0', zorder=3)
...
fig.text(0.05, 0.972, 'La toponimia del departamento en doce nóminas oficiales, 1909--1940',
...
fig.text(0.05, 0.948, 'Cada columna es una lista que el Boletín Oficial publicó; un cuadrado indica que ese nombre figura en ella. En negrita, los nombres\n'
         'que están en once o en las doce.', fontsize=7, ha='left', color='#333333', linespacing=1.5)

leg = [Line2D([], [], marker='s', ls='', ms=4.0, color='#1f4e5f', label='En once o doce de las doce listas'),
       Line2D([], [], marker='s', ls='', ms=4.0, color='#5a8ca0', label='En diez o menos'),
...
fig.text(0.712, 0.322, 'Variantes de grafía registradas:', fontsize=6.8, va='top', weight='bold')
fig.text(0.712, 0.306, '\n'.join(textwrap.fill('· ' + v, 44, subsequent_indent='  ') for v in var),
...
fig.text(0.05, 0.042,
 ...
 'Nº 3 de 1918 a 1923, del Nº 5 de 1926 y 1927 ---que ya no cubre Vaqueros, desprendido en el Nº 6--- y del Nº 2 de 1938\n'
 'y 1940, que son la misma lista nombre por nombre (Boletín\n'
 'Oficial de Salta; los once años están leídos edición por edición, y 1922 sólo hasta el 21 de julio). Ubicación: campo «finca»\n'
